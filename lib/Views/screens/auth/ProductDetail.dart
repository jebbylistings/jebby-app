...
import 'package:jebby/Views/screens/home/reviews.dart';
...
class _ProductDetailScreenState extends State<ProductDetailScreen> {
        ...
        if (this.mounted)
          {
            if (list.data!.length == 0)
            ...
            else
              {
                setState(() {
                  pastart =
                      ApiRepository.shared.getProductsByIdList?.data![0].pastart
                          .toString();
                  paend =
                      ApiRepository.shared.getProductsByIdList?.data![0].paend
                          .toString();
                  length =
  ...
  void preOrder(neg, req, context) {
    ...
    final snackBar = new SnackBar(
      content: new Text("Price is not in range defined"),
    );
    if (requested >= amount && requested <= price) {
      ApiRepository.shared.negotiationRequest(
        widget.id,
        sourceId,
        requested,
        context,
        ...
        child: SingleChildScrollView(
          child: Column(
            children: [
              ...
              isLoading
                  ? SizedBox(height: 10)
                  : ApiRepository.shared.getRelatedProductsList!.data!.length >
                      0
                  ? FutureBuilder(
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              SizedBox(height: 2 * 0.005),
              Container(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('$tx', style: TextStyle(fontSize: 11)),
                    SizedBox(height: 2 * 0.006),
                    Text(
                      '$dx',
